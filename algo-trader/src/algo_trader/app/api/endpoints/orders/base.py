...
from algo_trader.app.api.clients.orders import serialise_order
from algo_trader.app.api.clients.models.orders import SerialisedOrder
    ...
    async def send_order_change(order: Trade):
        if websocket.application_state == WebSocketState.CONNECTED:
            await websocket.send_json(
                {"type": "change", "data": await serialise_order(order)}
            )

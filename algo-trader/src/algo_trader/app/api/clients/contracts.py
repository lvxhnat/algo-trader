...
async def serialise_tickerdata(ticker_data: Ticker, status: MarketStatus):
    ...
    try:
        ...
        response["last"] = check_nan(ticker_data.close) if math.isnan(ticker_data.last) else ticker_data.last
        ...
        response["dividends"] = {
            ...
            "next_date": ticker_data.dividends.nextDate.strftime("%Y-%m-%d") if ticker_data.dividends.nextDate else None,
            "next_amount": ticker_data.dividends.nextAmount
        } if ticker_data.dividends else None
        return response

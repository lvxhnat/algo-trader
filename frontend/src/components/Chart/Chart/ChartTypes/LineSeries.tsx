...
export default function LineSeries(props: LineSeriesProps) {
  ...
  useEffect(() => {
    ...
    const line = d3.line<OHLCVData>()
    .x(function(_, i) { return xScale(data[i].date)!; })
    .y(function(_, i) { return yScale(data[i].close); })
    .defined((_, i) => !!data[i]);

    svg.append("path")
      .attr("fill", "none")
      .attr("stroke", theme.mode === 'dark' ? ColorsEnum.white : ColorsEnum.black)
      .attr("stroke-width", 1)
      .attr("d", line(data));
